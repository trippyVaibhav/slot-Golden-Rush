        ...
        : AbstractFpCurve
    {
            ...
            : base(q)
        {
            if (!isInternal)
            {
                ...
                if (unknownQ)
                {
                    int maxBitLength = AsInteger("Best.HTTP.SecureProtocol.Org.BouncyCastle.EC.Fp_MaxSize", 1042); // 2 * 521
                    int certainty = AsInteger("Best.HTTP.SecureProtocol.Org.BouncyCastle.EC.Fp_Certainty", 100);
