...
namespace Best.SocketIO.Events
{
    ...
    {
        [Best.HTTP.Shared.PlatformSupport.IL2CPP.Preserve] public string sid;
    }
    ...
    public struct CallbackDescriptor
    {
        ...
        public override string ToString()
            => $"[CallbackDescriptor {Once}, {this.ParamTypes?.Length}, {this.Callback}]";
    }
    ...
    /// <summary>
    /// Manages subscriptions for a <see cref="Socket"/>.
    /// </summary>
            ...
            if (this.subscriptions.TryGetValue(eventName, out subscription))
            {
                for (int i = 0; i < subscription.callbacks.Count; ++i)
                {
                    ...
                    if (callbackDesc.Once)
                    {
                        ...
                        // If no more subscribers left, remove the whole subscription.
