...
namespace Best.SocketIO.Parsers
{
    ...
    public sealed class DefaultJsonParser : IParser
    {
        ...
        {
            Best.HTTP.JSON.LitJson.JsonMapper.RegisterImporter<string, byte[]>(str => Convert.FromBase64String(str));
        }
        ...
        private (string, object[]) ReadData(SocketManager manager, IncomingPacket packet, string payload)
        {
            ...
            switch (packet.SocketIOEvent)
            {
                ...
                case SocketIOEventTypes.BinaryAck:
                    ...
                    args = ReadParameters(socket, subscription, HTTP.JSON.LitJson.JsonMapper.ToObject<List<object>>(payload), 0);
                ...
                default:
                    ...
                    using (var reader = new System.IO.StringReader(payload))
                        array = HTTP.JSON.LitJson.JsonMapper.ToObject<List<object>>(new HTTP.JSON.LitJson.JsonReader(reader));
        ...
        public object ConvertTo(Type toType, object obj)
        {
            ...
            return HTTP.JSON.LitJson.JsonMapper.ToObject(toType, HTTP.JSON.LitJson.JsonMapper.ToJson(obj));
        }
            ...
            if (paramCount > 0)
            {
                ...
                for (int i = 0; i < desc.ParamTypes.Length; ++i)
                {
                    ...
                    if (type == typeof(Socket))
                        ...
                        Best.HTTP.JSON.LitJson.JsonReader jr = new HTTP.JSON.LitJson.JsonReader(reader);
                        args[i] = HTTP.JSON.LitJson.JsonMapper.ToObject(desc.ParamTypes[i], jr);
                        reader.Read();
                    }
                }
            }
            ...
            switch (socketIOEvent)
            {
                case SocketIOEventTypes.Connect:
                    ...
                    if (args != null && args.Length > 0)
                    {
                        ...
                        builder.Append(Best.HTTP.JSON.LitJson.JsonMapper.ToJson(args[0]));
                    }
                    break;
                ...
                case SocketIOEventTypes.BinaryAck:
                    ...
                    if (args != null && args.Length > 0)
                    {
                        var argsJson = HTTP.JSON.LitJson.JsonMapper.ToJson(args);
