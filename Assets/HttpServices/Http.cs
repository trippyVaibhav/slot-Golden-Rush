...
namespace Duck.Http
{
	public sealed class Http : MonoBehaviour
	{
		...
		private Dictionary<IHttpRequest, Coroutine> httpRequests=new Dictionary<IHttpRequest, Coroutine>();
		...
		internal void Send(IHttpRequest request, Action<HttpResponse> onSuccess = null,
			Action<HttpResponse> onError = null, Action<HttpResponse> onNetworkError = null,string reqMethod="")
		{
			var enumerator = SendCoroutine(request, onSuccess, onError, onNetworkError, reqMethod);
		...
		private IEnumerator SendCoroutine(IHttpRequest request, Action<HttpResponse> onSuccess = null,
			Action<HttpResponse> onError = null, Action<HttpResponse> onNetworkError = null, string reqMethod = "")
		{
			yield return service.Send(request, onSuccess, onError, onNetworkError, reqMethod);
            try
            {
            ...
            {
                Debug.Log("Exception in removing request");
            }
        }
		...
		internal void Abort(IHttpRequest request)
		{
            ...
            {
                Debug.Log("Exception in aborting request");
            }
		}
